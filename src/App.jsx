      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px"}}>
        ...
        {!adding && editingIdx===null && (
          <button onClick={()=>setAdding(true)} style={{
            width:"100%",padding:"12px",borderRadius:14,border:"2px dashed #c4b5fd",
            background:"none",color:"#7c3aed",fontSize:14,fontWeight:800,cursor:"pointer",
            fontFamily:"'Nunito',sans-serif",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"center",gap:6
        ...
        {(adding || editingIdx!==null) && (
          <div style={{background:"#fff",borderRadius:18,padding:16,marginBottom:16,boxShadow:"0 2px 14px rgba(0,0,0,0.08)"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
              {EMOJIS.map(e=>(
                <button key={e} onClick={()=>setNewEmoji(e)} style={{
                  width:34,height:34,borderRadius:10,border: newEmoji===e?"2px solid #7c3aed":"1.5px solid #ede8ff",
                  background: newEmoji===e?"#f0eaff":"#faf8ff",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"
                }}>{e}</button>
              ))}
            </div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
              {PALETTE.map(c=>(
                <button key={c} onClick={()=>setNewColor(c)} style={{
                  ...
                  border: newColor===c?"3px solid #7c3aed":"2px solid rgba(0,0,0,0.08)",cursor:"pointer"
                }} />
              ))}
            </div>
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={editingIdx!==null?saveEdit:addNew} style={{flex:2,padding:"10px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#b79cff,#7ec8ff)",color:"#fff",fontWeight:900,fontSize:13,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>
                {editingIdx!==null?"Save changes":"Add category"}
              </button>
            </div>
          </div>
        )}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {cats.map((cat,i)=>(
            <div key={i} style={{
              ...
              border: editingIdx===i?"2px solid #b79cff":"2px solid transparent"
            }}>
...
function AddModal({ categories, sizes, onAdd, onClose }) {
  ...
  function submit() {
    if (!type) return;
    onAdd({ id:Date.now(), type, size, qty:Number(qty), label, image });
    onClose();
  }
  ...
  return (
    <Overlay>
      <Sheet style={{maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <PrimaryBtn onClick={submit} style={{marginTop:8}}>Add to Wardrobe 🎀</PrimaryBtn>
      </Sheet>
    </Overlay>
  );
}
      ...
      <div style={{height:80,background:cat?.color||"#f0eaff",position:"relative",overflow:"hidden",display:"flex"}}>
        {previewImgs.length>0
          ? previewImgs.map((item,i)=>(
              <img key={item.id} src={item.image} alt="" style={{flex:1,minWidth:0,objectFit:"cover",height:"100%",borderRight:i<previewImgs.length-1?"2px solid rgba(255,255,255,0.5)":"none"}} />
            ))
        ...
        {!isComplete && toBuy>0 && (
          <span style={{position:"absolute",top:7,right:7,background:"#ff7eb6",color:"#fff",borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:800}}>🛍 {toBuy}</span>
        )}
        {previewImgs.length>0 && group.items.length>previewImgs.length && (
          <span style={{position:"absolute",bottom:6,left:8,background:"rgba(0,0,0,0.35)",color:"#fff",borderRadius:20,padding:"1px 7px",fontSize:10,fontWeight:800}}>+{group.items.length-previewImgs.length} more</span>
        )}
      </div>
      <div style={{padding:"10px 12px 12px"}}>
        ...
        {pct!==null && (
          <div style={{marginBottom:6}}>
            <div style={{height:5,borderRadius:5,background:"#f0eaff",overflow:"hidden"}}>
              <div style={{height:"100%",width:`${pct}%`,borderRadius:5,background:isComplete?"#6ee7b7":"linear-gradient(90deg,#b79cff,#7ec8ff)"}} />
            </div>
          </div>
        )}
...
function ShoppingList({ groups, catMap }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {needed.map(g=>{
        const toBuy = g.goal-g.owned;
        const cat = catMap[g.type]||{};
        return (
          <div key={groupKey(g.type,g.size)} style={{background:"#fff",borderRadius:16,padding:"13px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 2px 10px rgba(0,0,0,0.06)"}}>
            <div style={{width:48,height:48,borderRadius:14,background:cat.color||"#f0eaff",flexShrink:0,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
              {g.items.find(i=>i.image)
                ? <img src={g.items.find(i=>i.image).image} alt="" style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.7}} />
...
const Sheet = ({children,style={}}) => (
  <div style={{background:"#fff",borderRadius:"28px 28px 0 0",padding:"20px 18px 36px",fontFamily:"'Nunito',sans-serif",boxShadow:"0 -8px 40px rgba(0,0,0,0.15)",...style}}>{children}</div>
);
...
const PrimaryBtn = ({children,onClick,style={}}) => <button onClick={onClick} style={{width:"100%",padding:13,borderRadius:14,border:"none",background:"linear-gradient(135deg,#b79cff,#7ec8ff)",color:"#fff",fontSize:15,fontWeight:900,cursor:"pointer",fontFamily:"'Nunito',sans-serif",boxShadow:"0 4px 18px rgba(140,100,255,0.3)",...style}}>{children}</button>;
...
const ImgPicker = ({image,onClick}) => (
  <div onClick={onClick} style={{width:"100%",height:110,borderRadius:18,border:"2px dashed #d8ceff",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",marginBottom:14,overflow:"hidden",background:"#faf8ff"}}>
    {image?<img src={image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} />
          :<div style={{textAlign:"center",color:"#c4b8f0"}}><div style={{fontSize:28}}>📷</div><div style={{fontSize:12,marginTop:3,fontWeight:700}}>Tap to upload photo</div></div>}
  </div>
);
...
export default function App() {
  ...
  const [sizes] = useState(DEFAULT_SIZES); // sizes are fixed but "One Size" is included
  ...
  function addItem(item) { setItems(prev=>[item,...prev]); }
  function deleteItem(id) { setItems(prev=>prev.filter(i=>i.id!==id)); }
  function saveGoal(type,size,goal) { setGoals(prev=>({...prev,[groupKey(type,size)]:goal})); }
  ...
  const filtered = Object.values(groups).filter(g=>
    ...
    (filterSize==="All"||g.size===filterSize)
  );
    ...
    return (
      <>
        ...
        <ManageCategories
          ...
          onSave={cats=>setCategories(cats)}
    ...
    <>
      ...
      <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",paddingBottom:90}}>
        ...
        <div style={{background:"linear-gradient(150deg,#b79cff 0%,#7ec8ff 100%)",padding:"32px 20px 22px",borderBottomLeftRadius:30,borderBottomRightRadius:30,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
            ...
            <button onClick={()=>setScreen("manage")} style={{background:"rgba(255,255,255,0.22)",border:"none",borderRadius:12,padding:"5px 12px",color:"#fff",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif",letterSpacing:0.3}}>⚙ Categories</button>
          </div>
          ...
          {tab==="wardrobe" && (<>
            {Object.keys(groups).length>0&&(<>
              <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:6}}>
                {usedTypes.map(t=>(
                  <button key={t} onClick={()=>setFilterType(t)} style={{flexShrink:0,padding:"5px 12px",borderRadius:20,border:"none",background:filterType===t?"#7c3aed":"#fff",color:filterType===t?"#fff":"#999",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>{t}</button>
                ))}
              </div>
              <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:14}}>
                {usedSizes.map(s=>(
                  <button key={s} onClick={()=>setFilterSize(s)} style={{flexShrink:0,padding:"5px 12px",borderRadius:20,border:"none",background:filterSize===s?"#7ec8ff":"#fff",color:filterSize===s?"#fff":"#999",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>{s}</button>
                ))}
              </div>
            </>)}
            {filtered.length===0
              ...
              : <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  {filtered.map(g=>(
                    <GroupCard key={groupKey(g.type,g.size)} group={g} cat={catMap[g.type]} onClick={()=>setDetail(g)} />
                  ))}
                </div>
            }
          </>)}
      ...
      {liveDetail && !goalFor && (
        <DetailDrawer
          ...
          onDeleteItem={id=>{deleteItem(id);if(liveDetail.items.length<=1)setDetail(null);}}
      ...
      {goalFor && (
        <GoalModal
          group={goalFor}
          catMap={catMap}
          onSave={goal=>saveGoal(goalFor.type,goalFor.size,goal)}
